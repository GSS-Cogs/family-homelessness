...
class GeoCodeFinder:
    ...
    def __init__(self, choices=None):
        ...
        self.graph = Graph()
        self.graph.parse("https://raw.githubusercontent.com/GSS-Cogs/GDP-vocabs/master/vocabs/reference-geography.ttl", format="ttl")
        
        lookups = {}
        for s, o, p in self.graph:
            if str(o) == 'http://statistics.data.gov.uk/def/statistical-geography#officialname' or \
                 str(o) == 'http://publishmydata.com/def/ontology/foi/displayName':
    ...
    def __call__(self, cell_val):
        ...
        if codes is None:
            if cell_val not in self.has_had_warning:
                logging.warning(f'Unable to find code for label {cell_val}.\nPlease specifiy what you want to use ' \
                               + f'by passing in a dictionary, eg "GeoCodeFinder(choices={{"{cell_val}":"Z123456789"}})"\n')
        ...
        if len(codes) > 1:
            if cell_val not in self.has_had_warning:
                logging.warning(f'The label {cell_val} has {len(codes)} possible codes as follows" {codes}.\nPlease specify which you want to use' \
                                + f'by passing in a dictionary, eg "GeoCodeFinder(choices={{"{cell_val}":"{codes[0]}"}})"\n')
                self.has_had_warning.append(cell_val)
            return cell_val
...
for tab in distro.as_databaker():
    ...
    try:
        ...
        trace.start("mhclg-families-in-bed-and-breakfast-accommodation-for-more-than-6-weeks", tab.name,
            ["Value", "Marker", "Period", "Family Accomodation" ,"Area"], distro.downloadURL)
        ...
        family_accomodation = anchor.expand(RIGHT) - area.expand(UP)
        trace.Family_Accomodation('Fields taken from alongside (but not including) "Authority".', excel_range(family_accomodation))

        trace.obs('Taken as cells that can be cross referenced from the "Area" and "Family Accomodation" selections.')
        obs = family_accomodation.waffle(area).is_not_blank().is_not_whitespace()

        dimensions = [
            HDim(period, "Period", CLOSEST, LEFT),
            HDim(family_accomodation, "Family Accomodation", DIRECTLY, ABOVE),
...
marker_frame1["Family Accomodation"] = 'Number of families in B&B accommodation for 6 or more weeks'
marker_frame2["Family Accomodation"] = 'Number of families in B&B accommodation for 6 or more weeks not pending a review or appeal'
df = pd.concat([data_frame, marker_frame1, marker_frame2])
trace.multi(["Family_Accomodation", "Marker"], f'Replace each "Authorities that did not submit a figure" with a "{missing_marker}" ' \
                            + 'against each Family Accomodation option')

df["Family Accomodation"] = df["Family Accomodation"].apply(pathify)
trace.Family_Accomodation('Pathify all values')
