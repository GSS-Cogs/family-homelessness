...
#         housing_type = prs_srs_1.shift(DOWN).expand(RIGHT)
#         observations = housing_type.fill(DOWN).shift(LEFT).fill(RIGHT).is_not_blank()-remove_notes
...
#         savepreviewhtml(household_type_and_composition, fname= tab.name + "PREVIEW.html")
...
#             HDim(housing_type, 'housing_type', DIRECTLY, ABOVE),
#             #HDimConst("sheet_name", sheet_name) #Might be handy to have for post processing when other tabs are running also 
#         ]
#         tidy_sheet = ConversionSegment(tab, dimensions, observations)
#         savepreviewhtml(tidy_sheet, fname= tab.name + "PREVIEW.html")
...
for tab in tabs:
    columns=['Contents']
    trace.start(datasetTitle, tab, columns, distribution.downloadURL)
    if tab.name in ['P5']: #only transforming tab A10 for now
        print(tab.name)
...
for tab in tabs:
    columns=['Contents']
    trace.start(datasetTitle, tab, columns, distribution.downloadURL)
    if tab.name in ['R1']: #only transforming tab A10 for now
        print(tab.name)
...
for tab in tabs:
    columns=['Contents']
    trace.start(datasetTitle, tab, columns, distribution.downloadURL)
    if tab.name in ['R2']: #only transforming tab A10 for now
        print(tab.name)
        ...
        quarter = unwanted.shift(LEFT)-unwanted
        period = quarter.shift(LEFT).is_not_blank()
        savepreviewhtml(household_relief_duty_ended_by_type_of_accommodation, fname= tab.name + "PREVIEW.html")
