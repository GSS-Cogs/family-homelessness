...
for tab in tabs:
    ...
    if tab.name in ['MD2']: #only transforming tab MD2 for now
        ...
        accepted_refused = household_by_decision_duty_owed.shift(DOWN).expand(RIGHT)
...
#         savepreviewhtml(period, fname= tab.name + "PREVIEW.html")
        dimensions = [
            HDim(quarter,'quarter',DIRECTLY,LEFT),
            HDim(period,'period',CLOSEST,ABOVE),
            HDim(household_owed_priority, 'household_main_duty_ended', DIRECTLY, ABOVE), 
            HDim(vulnerable_household, 'accepted_refused', DIRECTLY, ABOVE),
        ]
        tidy_sheet = ConversionSegment(tab, dimensions, observations)
        savepreviewhtml(tidy_sheet, fname= tab.name + "PREVIEW.html")
        trace.with_preview(tidy_sheet)
