...
reason = cell.fill(RIGHT).is_not_blank().is_not_whitespace() | tab.excel_ref('P5:Q5')
...
new_table['Reason for Homelessness'] = new_table['Reason for Homelessness'].map(
    lambda x: {'Duty discharged' : 'Status of Household Duty Discharged',
       'Live full duty applicants' : 'Status of Household Live full duty applicants'        
        }.get(x, x))
# -
...
tab = tabs['2_5']
cell = tab.excel_ref('A2')
category = cell.fill(RIGHT).is_not_blank().is_not_whitespace()
year = cell.fill(DOWN).is_not_blank().is_not_whitespace()  
observations = year.fill(RIGHT).is_not_blank().is_not_whitespace() 
Dimensions = [
            HDim(category,'Priority Need Category',DIRECTLY,ABOVE),
            HDim(year, 'Period',DIRECTLY,LEFT),
            HDimConst('Unit','Count'),  
            HDimConst('Measure Type','Household')
...
observations = year.fill(RIGHT).is_not_blank().is_not_whitespace() 
Dimensions = [
            HDim(outcome,'Housing Assessment Outcome',DIRECTLY,ABOVE),
            HDim(year, 'Period',DIRECTLY,LEFT),
            HDimConst('Unit','Count'),  
            HDimConst('Measure Type','Household')
...
next_table.drop_duplicates().to_csv(out / 'nihe-homelessness-acceptance.csv', index = False)
